...
template<class vertex>
void sampling_edge(graph<vertex> &G, graph<vertex> &G1, string prefix){
    std::vector<uintE> rank = rank_10_out(G1);
    std::vector<uintE> rank_in = rank_10_in(G1);
    ...
    for (auto i = 0; i < 10; i++)
    {
        ...
        {
          intE src = j;
          intE degreeV = G.V[src].getOutDegree();
            if (query_result_in[i][src] != 65536)
            {
              for (int k = 0; k < degreeV; k++){
                intE dst = G.V[j].getOutNeighbor(k);
                intE edgeLen = (src + dst) % 16 + 1;
                if(query_result_in[i][src] + edgeLen == query_result_in[i][dst]){
                  ...
                  {
                    edge_set.insert(std::make_pair(dst, src));
                    in_flag[src] = 1;
                    out_flag[dst] = 1;              
                  }
                  }
    ...
    free(query_result);
    ofstream of(prefix + ".core_unweight");
...
void Compute(graph<vertex>& GA, commandLine P) {
    ...
    readGraph<asymmetricVertex>(iFile1,false,false,false,false);
    std::string outFile = std::string(iFile);
    sampling_edge(GA, G1, outFile);
}